...
7. [컴포넌트 상태](#7-컴포넌트-상태)
8. [그리드 시스템](#8-그리드-시스템)
9. [모션/애니메이션](#9-모션애니메이션)
10. [레이아웃 패턴](#10-레이아웃-패턴)
11. [다크 모드](#11-다크-모드)
12. [접근성 가이드](#12-접근성-가이드)
13. [TailwindCSS 설정](#13-tailwindcss-설정)
14. [디자인 토큰](#14-디자인-토큰)
...
모든 인터랙티브 컴포넌트는 다음 상태를 지원합니다:

| 상태 | 설명 | 시각적 표현 |
|------|------|------------|
| **Default** | 기본 상태 | 표준 스타일 |
| **Hover** | 마우스 오버 | 밝기/색상 변화 |
| **Active/Pressed** | 클릭/터치 중 | 더 어두운 색상 |
| **Focus** | 키보드 포커스 | 포커스 링 표시 |
| **Disabled** | 비활성화 | 투명도 50%, 커서 변경 |
| **Loading** | 로딩 중 | 스피너 표시 |
...
<!-- Focus -->
<button class="bg-primary-500 focus:outline-none focus:ring-2
               focus:ring-primary-500 focus:ring-offset-2">
  포커스
</button>

<!-- Disabled -->
<button class="bg-primary-500 opacity-50 cursor-not-allowed" disabled>
  비활성
</button>

<!-- Loading -->
<button class="bg-primary-500 cursor-wait" disabled>
...
```css
/* 모든 상태 전환에 적용 */
transition-colors duration-200  /* 색상 변화: 200ms */
transition-all duration-200     /* 전체 변화: 200ms */
```
...
```html
<!-- Default -->
<input class="border-cream-300 bg-cream-100">

<!-- Focus -->
<input class="focus:border-primary-400 focus:ring-2 focus:ring-primary-200">

<!-- Error -->
<input class="border-red-400 bg-red-50 focus:border-red-500 focus:ring-red-200">

<!-- Success -->
<input class="border-emerald-400 bg-emerald-50">

<!-- Disabled -->
<input class="bg-warm-gray-100 text-warm-gray-400 cursor-not-allowed" disabled>

<!-- Read-only -->
<input class="bg-warm-gray-50 cursor-default" readonly>
```
...
<!-- Hover -->
<div class="bg-white hover:bg-cream-50 transition-colors">

<!-- Active/Selected -->
<div class="bg-primary-50 border-primary-200">

<!-- Disabled -->
<div class="bg-warm-gray-50 opacity-60 pointer-events-none">
```
...
<%= heroicon "home", variant: :outline,
    options: { class: "w-6 h-6 text-warm-gray-500" } %>

<%# Active (활성) %>
<%= heroicon "home", variant: :solid,
    options: { class: "w-6 h-6 text-primary-500" } %>
```
...
모아봄은 모바일 우선 반응형 그리드를 사용합니다.

| 브레이크포인트 | 크기 | 컬럼 수 | 거터 |
|---------------|------|---------|------|
| 모바일 (기본) | < 640px | 2-3 | 8-16px |
| `sm:` | >= 640px | 3-4 | 16px |
| `md:` | >= 768px | 4-6 | 20px |
| `lg:` | >= 1024px | 6-8 | 24px |
...
#### 2열 카드 그리드
```html
<div class="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-3 lg:grid-cols-4 px-4">
  <!-- 카드 아이템 -->
</div>
```

#### Masonry 스타일 (Bento Grid)
```html
<div class="grid grid-cols-2 md:grid-cols-3 gap-2 px-4">
  <!-- 2칸 차지하는 큰 아이템 -->
  <div class="col-span-2 row-span-2 aspect-video rounded-2xl overflow-hidden">
    <img src="..." class="w-full h-full object-cover">
  </div>
...
```html
<!-- 세로 리스트 -->
<div class="flex flex-col divide-y divide-cream-200">
  <div class="py-4 px-4">아이템 1</div>
  <div class="py-4 px-4">아이템 2</div>
</div>

<!-- 가로 스크롤 리스트 -->
<div class="flex gap-3 overflow-x-auto px-4 pb-2 scrollbar-hide">
  <div class="flex-shrink-0 w-32">아이템 1</div>
  <div class="flex-shrink-0 w-32">아이템 2</div>
</div>
```
...
```html
<!-- 단일 컬럼 (모바일) -->
<form class="space-y-4 px-4 max-w-lg mx-auto">
  <div>필드 1</div>
  <div>필드 2</div>
</form>

<!-- 2컬럼 (태블릿+) -->
<form class="grid grid-cols-1 md:grid-cols-2 gap-4 px-4">
  <div>필드 1</div>
  <div>필드 2</div>
  <div class="md:col-span-2">전체 너비 필드</div>
</form>
```
...
```html
<!-- 최대 너비 컨테이너 -->
<div class="max-w-lg mx-auto px-4">  <!-- 모바일 폼 -->
<div class="max-w-2xl mx-auto px-4">  <!-- 콘텐츠 -->
<div class="max-w-4xl mx-auto px-4">  <!-- 대시보드 -->
<div class="max-w-6xl mx-auto px-4">  <!-- 풀 레이아웃 -->
...
| 기본 | 200ms | `ease-in-out` | 상태 변화, 색상 |
| 느린 | 300ms | `ease-in-out` | 레이아웃 변화 |
| 엔터 | 300ms | `ease-out` | 모달, 페이드인 |
| 엑싯 | 200ms | `ease-in` | 닫기, 페이드아웃 |
...
```html
<button class="group">
  <svg class="w-6 h-6 group-active:scale-125 transition-transform duration-200">
    <!-- 하트 아이콘 -->
  </svg>
</button>
```
...
#### 카드 호버 효과
```html
<div class="hover:scale-[1.02] hover:shadow-lg transition-all duration-200">
  <!-- 카드 콘텐츠 -->
</div>
```
...
#### 펄스 (스켈레톤)
```html
<div class="animate-pulse bg-cream-200 rounded-xl h-4 w-3/4"></div>
```
...
<!-- 모달 슬라이드업 -->
<div class="fixed bottom-0 inset-x-0 bg-white rounded-t-3xl
...
디자인 토큰은 디자인 시스템의 기본 값들을 구조화된 형식으로 정의합니다.
...
```json
{
  "color": {
    "primary": {
      ...
      "300": { "value": "#FFBFA3" },
      "400": { "value": "#FFA885" },
      "500": { "value": "#FF8B66", "description": "메인 브랜드 색상" },
      "600": { "value": "#E67350" },
      "700": { "value": "#CC5A3A" },
      "800": { "value": "#B34425" },
      "900": { "value": "#992E10" }
    ...
    "secondary": {
      ...
      "100": { "value": "#CCFFF5" },
      "200": { "value": "#99FFEB" },
      "500": { "value": "#4ECDC4", "description": "보조 색상" },
      "600": { "value": "#3DB5AD" },
      "700": { "value": "#2D9E97" }
    },
    "accent": {
      "100": { "value": "#FFE0E6" },
      "500": { "value": "#FF6B9D", "description": "강조 색상" },
      "600": { "value": "#E65585" }
    },
    "cream": {
      "50": { "value": "#FFFDFB", "description": "페이지 배경" },
      "100": { "value": "#FFF9F5" },
      "200": { "value": "#FFF3EB" },
      "300": { "value": "#FFEDE0" }
    },
    "warm-gray": {
      ...
      "300": { "value": "#D6D3D1" },
      "400": { "value": "#A8A29E" },
      "500": { "value": "#78716C", "description": "보조 텍스트" },
      "600": { "value": "#57534E" },
      "700": { "value": "#44403C", "description": "본문 텍스트" },
      "800": { "value": "#292524", "description": "제목 텍스트" },
      "900": { "value": "#1C1917" }
    },
    "semantic": {
...
spacing:
  ...
  semantic:
    ...
    grid-gap-tight: 2px     # gap-0.5
    grid-gap-normal: 16px   # gap-4
```
...
```json
{
  "typography": {
    "fontFamily": {
      "sans": "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, 'Noto Sans KR', sans-serif"
    },
    "fontSize": {
...
components:
  ...
  card:
    ...
    elevated:
      ...
      border-radius: "24px"
      shadow: "0 4px 20px rgba(255, 139, 102, 0.12)"
...
const buttonPadding = tokens.components.button.padding.md;
```
...
| 1.1 | 2025-12-16 | 컴포넌트 상태, 그리드 시스템, 모션/애니메이션, 디자인 토큰 섹션 추가 |
