...
---
In test/system/families/members_test.rb around lines 113 to 116, the loop over
@family.family_memberships is redundant because assert_no_selector
"button[aria-label*='관리']" checks the whole page; remove the each loop and
replace it with a single global assertion assert_no_selector
"button[aria-label*='관리']" (i.e., delete the iteration and keep one assertion)
so the test expresses the intent without repeating the same page-level check
multiple times.