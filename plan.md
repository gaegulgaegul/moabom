...
- [x] **RED**: 배치 업로드 검증 테스트 ✅
  - [x] 빈 배열 요청 시 400 응답
  - [x] 배열이 아닌 요청 시 400 응답
  - [x] 최대 개수 초과 시 400 응답
- [x] **GREEN**: batch 액션에 입력 검증 추가 ✅
  - [x] photos 파라미터 존재 여부 확인
  - [x] 배열 타입 검증
  - [x] MAX_BATCH_SIZE 상수 정의 및 검증
  - [x] 빈 배열/빈 문자열 포함 검증 (Rails params 동작 대응)
- [x] **REFACTOR**: 검증 로직 메서드 추출 ✅ (validate_batch_params로 분리 완료)
...
- [x] **RED**: 이모지 검증 테스트 ✅ 2025-12-15
  - [x] 허용된 이모지만 저장 가능
  - [x] 허용되지 않은 문자열 거부
  - [x] 빈 값 거부
- [x] **GREEN**: Reaction 모델에 이모지 검증 추가 ✅ 2025-12-15
  - [x] ALLOWED_EMOJIS 상수 정의
  - [x] validates :emoji, inclusion: { in: ALLOWED_EMOJIS }
- [x] **REFACTOR**: 코드 정리 ✅ 2025-12-15 (변경 없음 - 코드 간결)

### 6.5.2 댓글 길이 제한

- [x] **RED**: 댓글 길이 제한 테스트 ✅ 2025-12-15
  - [x] 1000자 이하 댓글 저장 가능
  - [x] 1000자 초과 시 거부
  - [x] 에러 메시지 확인
- [x] **GREEN**: Comment 모델에 길이 검증 추가 ✅ 2025-12-15
  - [x] validates :body, length: { maximum: MAX_BODY_LENGTH }
  - [x] MAX_BODY_LENGTH 상수 정의
- [x] **REFACTOR**: 코드 정리 ✅ 2025-12-15 (변경 없음 - 코드 간결)

### 6.5.3 허용 이모지 목록 정의

- [x] **GREEN**: 이모지 목록 구현 ✅ 2025-12-15
  - [x] Reaction::ALLOWED_EMOJIS 정의 (8개 기본 이모지)
- [x] **REFACTOR**: 코드 정리 ✅ 2025-12-15 (변경 없음 - 모델 상수로 충분)

### 6.5.4 잘못된 이모지 입력 처리

- [x] **RED**: 잘못된 이모지 에러 처리 테스트 ✅ 2025-12-15
  - [x] 유효하지 않은 이모지 요청 시 422 응답
  - [x] 사용자 친화적 에러 메시지 반환
- [x] **GREEN**: 에러 처리 확인 ✅ 2025-12-15 (이미 구현됨)
  - [x] 검증 실패 시 에러 응답
- [x] **REFACTOR**: 코드 정리 ✅ 2025-12-15 (변경 없음 - 기존 구현 활용)

### 6.5.5 긴 댓글 입력 처리

- [x] **RED**: 긴 댓글 에러 처리 테스트 ✅ 2025-12-15
  - [x] 길이 초과 시 422 응답
  - [x] 현재 길이 및 최대 길이 안내
- [x] **GREEN**: 에러 처리 확인 ✅ 2025-12-15 (이미 구현됨)
  - [x] 검증 에러 응답 처리
...
- [x] **RED**: UI 스타일 테스트 ✅ 2025-12-15
  - [x] 프로필 페이지에 TailwindCSS 클래스 적용 확인
  - [x] 알림 설정 페이지 스타일 확인
  - [x] 반응형 레이아웃 동작 확인
- [x] **GREEN**: TailwindCSS 스타일 적용 ✅ 2025-12-15
  - [x] 프로필 폼 스타일링
  - [x] 알림 설정 토글 스타일링
  - [x] 에러 메시지 스타일 개선
- [x] **REFACTOR**: 변경 없음 (코드 간결) ✅ 2025-12-15
...
- [x] **RED**: i18n 테스트 ✅ 2025-12-15
  - [x] flash 메시지가 i18n 키로 제공
  - [x] 에러 메시지 한국어로 표시
  - [x] 설정 페이지 레이블 i18n
- [x] **GREEN**: i18n 파일 구성 ✅ 2025-12-15
  - [x] ko.yml에 설정 관련 번역 추가
  - [x] 컨트롤러 flash 메시지 i18n 적용
  - [x] 뷰 텍스트 i18n 적용
- [x] **REFACTOR**: 변경 없음 (코드 간결) ✅ 2025-12-15

### 7.5.4 뷰 레이블 i18n

- [x] **RED**: 뷰 레이블 i18n 테스트 ✅ 2025-12-15
  - [x] form_for 레이블 i18n
  - [x] 버튼 텍스트 i18n
  - [x] 안내 문구 i18n
- [x] **GREEN**: 뷰에 i18n 헬퍼 적용 ✅ 2025-12-15
  - [x] t(".label_name") 형식 사용
  - [x] model attribute 번역 설정
- [x] **REFACTOR**: 변경 없음 (코드 간결) ✅ 2025-12-15
...
- [x] **RED**: 아바타 파일 검증 테스트 ✅ 2025-12-15
  - [x] 허용 타입 (jpeg, png, webp)만 업로드 가능
  - [x] 파일 크기 5MB 제한
  - [x] 잘못된 파일 에러 메시지
- [x] **GREEN**: User 모델에 아바타 검증 추가 ✅ 2025-12-15
  - [x] content_type 검증
  - [x] byte_size 검증
- [x] **REFACTOR**: 파일 검증 concern 추출 ✅ 2025-12-15 (변경 없음 - 코드 간결)
...
- [x] **RED**: 상태 화면 테스트 ✅ 2025-12-15
  - [x] 사진 없음 상태
  - [x] 404 페이지
  - [x] 500 페이지 (404와 동일한 패턴 적용)
- [x] **GREEN**: 상태 화면 구현 ✅ 2025-12-15
- [x] **REFACTOR**: 변경 없음 (코드 간결) ✅ 2025-12-15

### 9.3 반응형 디자인

- [x] **RED**: 반응형 테스트 ✅ 2025-12-15
  - [x] 모바일 레이아웃 (container, grid 클래스)
  - [x] 태블릿 레이아웃 (md:, lg: breakpoints)
- [x] **GREEN**: TailwindCSS 반응형 적용 ✅ 2025-12-15
- [x] **REFACTOR**: 변경 없음 (코드 간결) ✅ 2025-12-15
...
| Phase 7.5: 설정 UI/검증 | ✅ 완료 | 2025-12-15 |
...
| Phase 9.2-9.3: UI/UX 마무리 | ✅ 완료 | 2025-12-15 |
